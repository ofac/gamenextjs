...
export default function EditConsole({ console }: any) {

    const [errors, setErrors]          = useState<any>({});
    const [preview, setPreview]        = useState(`/imgs/${console.image}`);
    ...
    const [form, setForm] = useState({
        name:         console.name,
        releasedate:  new Date(console.releasedate).toISOString().split('T')[0],
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      startTransition(async () => {
          const res = await updateConsole(formData);
    ...
    return (
            ...
            {showToast && (
                <div className="toast toast-top toast-end">
                    <div className="alert alert-success">
                        <span>Console: <strong>{form.name}</strong> updated successfully</span>
                    </div>
                </div>
            )}
      <form
      ...
      >
        <fieldset disabled={showToast || isPending} className="contents">
          ...
          <input type="hidden" name="existing_image" value={console.image} />
          {/* Image - - - */}
          ...
          <div>
            ...
            >
              ...
              {isPending ? "Saving..." : showToast ? "Redirecting..." : "Edit Console"}
            </button>
          </fieldset>
      </form>
    </div>
